...
addExpense = (res) => {
  ...
  expenseItem.innerHTML = `
        <span>${res.category} - </span>
          <span>${res.quantity} items - </span>
          <span>${res.amount}</span>
          <button class="delete-btn" id="${res._id}">Delete</button>
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const res = await axios.post(`${URL}`, { numOfExpenses, numOfExpenses }, {
    headers: {
      "auth": token
    }
  })
...
document.querySelector('.pageNumber').addEventListener('click', async (e) => {
  ...
  //console.log(page)
  const result = await axios.post(`${URL}?page=${page}`, { numOfExpenses: numOfExpenses }, {
    headers: {
      'auth': token,
    }
  }
